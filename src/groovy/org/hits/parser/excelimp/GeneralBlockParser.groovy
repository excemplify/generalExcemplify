 ...
 * @author rongji
 */
class GeneralBlockParser implements Parser{
    ...
    def configure(StateAndQueue state, Map configurations) throws ParserConfigException{
        ...
        this.action=GeneralMatrixParserAction.valueOf(configurations.action)
        
        this.parsingway=configurations.parsingWay
        
    
        if (configurations.sources.size()>1){
            println "block parser configuring multi sources"
            source= new GeneralMultiExcelCellsSource(configurations.sources, state)
         
            println configurations.action
            if (action==GeneralMatrixParserAction.OUTERPRODUCT_COLUMNS_BLOCK) source.setSourceType(SourceType.OUTERPRODUCT)
            else source.setSourceType(SourceType.CONCAT)
        }
        else source = new GeneralExcelCellSource(configurations.sources.first(), state)
